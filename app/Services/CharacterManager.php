    ...
    */
    ...
    public function createCharacter($data, $user, $isMyo = false)
    {
        ...
        try {
			...
			if(isset($data['subtype_id_3']) && $data['subtype_id_3'])
            {
                $subtypeThree = Subtype::find($data['subtype_id_3']);
                if(!(isset($data['species_id']) && $data['species_id'])) throw new \Exception('Species must be selected to select a subtype.');
                if(!$subtypeThree || $subtype->species_id != $data['species_id']) throw new \Exception('Selected third subtype invalid or does not match species.');
            }
            else $data['subtype_id_3'] = null;
    ...
    private function handleCharacter($data, $isMyo = false)
    {
        try {
            if($isMyo)
            {
				...
				$data['subtype_id_3'] = isset($data['subtype_id_3']) && $data['subtype_id_3'] ? $data['subtype_id_3'] : null;
    ...
    private function handleCharacterImage($data, $character, $isMyo = false)
    {
        try {
            ...
            $imageData = Arr::only($data, [
                'species_id', 'subtype_id', 'subtype_id_2', 'subtype_id_3', 'rarity_id', 'use_cropper',
                'x0', 'x1', 'y0', 'y1',
            ]);
    ...
    public function createImage($data, $character, $user)
    {
        ...
        try {
			...
			if(isset($data['subtype_id_3']) && $data['subtype_id_3'])
            {
                $subtypeThree = Subtype::find($data['subtype_id_3']);
                if(!(isset($data['species_id']) && $data['species_id'])) throw new \Exception('Species must be selected to select a subtype.');
                if(!$subtypeThree || $subtypeThree->species_id != $data['species_id']) throw new \Exception('Selected third subtype invalid or does not match species.');
            }
            else $data['subtype_id_3'] = null;
    ...
    public function updateImageFeatures($data, $image, $user)
    {
        ...
        try {
			...
			$old['subtype_3'] = $image->subtype_id_3 ? $image->subtypeThree->displayName : null;
			...
			$image->subtype_id_3 = $data['subtype_id_3'] ?: null;
			...
			$new['subtype_3'] = $image->subtype_id_3 ? $image->subtypeThree->displayName : null;
...
is_object($sender) ? $sender->id : null,
    ...
    public function createDesignUpdateRequest($character, $user)
    {
        ...
        try {
            ...
            $data = [
				...
				'subtype_id_3' => $character->image->subtype_id_3,
            ];
    ...
    public function saveRequestFeatures($data, $request)
    {
        ...
        try {
			...
			if(isset($data['subtype_id_3']) && $data['subtype_id_3'])
                $subtypeThree = ($request->character->is_myo_slot && $request->character->image->subtype_id_3) ? $request->character->image->subtypeThree : Subtype::find($data['subtype_id_3']);
            else $subtypeThree = null;
			...
			if($subtypeThree && $subtypeThree->species_id != $species->id) throw new \Exception("Third subtype does not match the species.");
			...
			$request->subtype_id_3 = $subtypeThree ? $subtypeThree->id : null;
    ...
    public function approveRequest($data, $request, $user)
    {
        ...
        try {
            ...
            $image = CharacterImage::create([
				...
				'subtype_id_3' => ($request->character->is_myo_slot && isset($request->character->image->subtype_id_3)) ? $request->character->image->subtype_id_3 : $request->subtype_id_3,
