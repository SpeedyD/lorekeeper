...
@endif
	...
	<div class="form-group" id="subtypes_3">
        {!! Form::label('Third Subtype (Optional)') !!} @if($isMyo) {!! add_help('This will lock the slot into a particular third subtype. Leave it blank if you would like to give the user a choice, or not select a subtype. The subtype must match the species selected above, and if no species is specified, the subtype will not be applied.') !!} @endif
        {!! Form::select('subtype_id_3', $subtypes, old('subtype_id_3'), ['class' => 'form-control', 'id' => 'subtype_3']) !!}
    </div>
...
<script>
    $( "#species" ).change(function() {
	  ...
	  $.ajax({
        type: "GET", url: "{{ url('admin/masterlist/check-subtype/3') }}?species="+species+"&myo="+myo, dataType: "text"
